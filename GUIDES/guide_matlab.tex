...
\item Another option is to store the \texttt{.m}-files in a directory
  of your choosing, e.g., a directory named \verb|m_files| To achieve
  this, start MATLAB and enter the following commands.
\begin{verbatim}
cd(userpath)
websave('mfiles.zip','http://go.osu.edu/mfiles')
unzip('mfiles.zip')
\end{verbatim}
  To access the \texttt{.m}-files in the future, navigate to the
  \verb|m_files| directory with
\begin{verbatim}
cd(userpath)
cd('m_files')
\end{verbatim}
