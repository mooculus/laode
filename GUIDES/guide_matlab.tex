...
\item One option is to store the \texttt{.m}-files in the folder that MATLAB
  starts in.  Running the following code inside MATLAB will achieve this.
...
\item Another option is to store the \texttt{.m}-files in a directory of your choosing, e.g., a directory named \verb|m_files|.  To achieve this, run the following code inside MATLAB.
\begin{verbatim}
cd(userpath)
websave('mfiles.zip','http://go.osu.edu/mfiles')
unzip('mfiles.zip')
