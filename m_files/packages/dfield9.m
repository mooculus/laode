function output = dfield9(action,input1,input2,input3)

% dfield9	is an interactive tool for studying single first order
%	differential equations.  When dfield9 is executed, a dfield9 Setup
...
%	mouse button is depressed in the dfield9 Display window, the
...
%  Last modified: April 2, 2003 ---> 11-11-2015
%  Modified by Hil G.E. Meijer, University of Twente, Netherlands
%  to work with recent Matlab versions. Copyright still owned by JCP.
%  Last modified 2016-01-15 by GJ Hebbink
  ...
  if strcmp(action,'initialize')
    ...
    % First we make sure that there is no other copy of dfield9
    % running, since this causes problems.	
    
    dfh = findobj('tag','dfield9');
    if ~isempty(dfh);
      qstring = {'There are some open dfield9 figures (perhaps invisible).  ';...
		 'What do you want to do?'};
      tstring = 'Only one copy of dfield9 can be open at one time.';
      b1str = 'Restart dfield9.';
	...
	ll = fgetl(fid);
	fclose(fid);
	if strcmp(ll,'%% Created by dfield9')
	  delete(fn)
	else
	  kk = [kk;k];
	end
    ...
    dfset = figure('name','dfield9 Setup',...
		   'tag','dfield9','visible','off',...
		   'user',ud);
    
    dfield9('figdefault',dfset);
    ...
    ud.h.der=uicontrol('pos',[eqleft+varw+equalw derbot eqlength texth],...
		       ...
		       'Callback',der,'visible','off','backgroundcolor',ecolor);
    ...
    butt(1) = uicontrol('style','push',...
			'pos',qwind,...
			'string','Quit',...
			'Callback','dfield9(''quit'')',...
			'visible','off');
    
    butt(2) = uicontrol('style','push',...
			'pos',rwind,...
			'string','Revert',...
			'Callback','dfield9(''revert'')',...
			'visible','off');
  
    butt(3) = uicontrol('style','push',...
			'pos',pwind,...
			'string','Proceed',...
			'Callback','dfield9(''proceed'')',...
			'visible','off');
  ...
  melddg = uimenu(mefile,'label','Load the default gallery',...
		  'Callback','dfield9(''loadsyst'',''default'');',...
		  'enable','on',...
		  'tag','load default','pos',6);
  meproceed = uimenu(mefile,'label','Proceed','Callback',...
		       'dfield9(''proceed'')','separator','on',...
		       'accelerator','G','pos',7);
    merevert = uimenu(mefile,'label','Revert','Callback',...
		      'dfield9(''revert'')','separator','off','pos',8);
    set(mepset,'vis','on','pos',9);
    set(meprev,'vis','on','label','Page Pre&view...','pos',10);
    set(meexp,'vis','on','pos',11,'separator','off');
    merestart = uimenu(mefile,'label','Restart dfield9','Callback',...
		       'dfield9(''restart'')','separator','on','pos',12);
    mequit = uimenu(mefile,'label','Quit dfield9','Callback',...
		    'dfield9(''quit'')','separator','on','pos',13);
    ...
    meadd = uimenu(sysmenu,'label','Add current equation to the gallery',...
		   'Callback','dfield9(''addgall'');','tag','add system');
    sep = 'on';
  ...
  elseif strcmp(action,'proceed')
    ...
    if any(isnan(WINvect))
      errmsg = ['One of the entries defining the display window ',...
		'is not a number.'];
      fprintf('\a')
      errordlg(errmsg,'dfield9 error','on');
      return
    end
    xstr = sud.c.xname;
    if isempty(xstr)
    ...
    if (sud.flag == 1)
      dfdisp = findobj('name','dfield9 Display');
      dud = get(dfdisp,'user');
	...
	if ~isempty(pn)
	  if isempty(pv) 
	    ...
	    xderivstr = dfield9('paraeval',pn,pv,derivstr);
	...
	return
      ...
      % Find dfield9 Display if it exists.	
      % If dfield9 Display exists, update it.  If it does not build it.
	...
	dfdisp = figure('name','dfield9 Display',...
			'interrupt','on',...
			'visible','off','tag','dfield9');
	dfield9('figdefault',dfdisp);
	dud = get(dfdisp,'user');	
	dud.syst = sud.c;
	dud.settings = sud.settings;
	fs = dud.fontsize;
...
% 			'drawmode','fast',...	
	...
	merestart = uimenu(fmenu,'label','Restart dfield9','Callback',...
			   'dfield9(''restart'')','separator','on','pos',8);
	
	mequit = uimenu(fmenu,'label','Quit dfield9',...
			'Callback','dfield9(''quit'')','separator','on');
	...
	solverstr = ['ud = get(gcf,''user'');',...
		     ...
		     'set(me,''checked'',''on'');',...
		     'set(gcf,''user'',ud);',...
		     'dfset = findobj(''name'',''dfield9 Setup'');',...
		     'sud = get(dfset,''user'');',...
	...
	meset   = uimenu(menu(1),'label','Window settings.',...
			 'Callback','dfield9(''settings'')');
	
	menu(6) = uimenu(menu(1),'label','Make the Display Window inactive.',...
			 'Callback','dfield9(''hotcold'')','separator','on');
	
	% View menu
	
	set(findobj(gcf,'label','&View'),'pos',4);
	set(findobj(gcf,'label','&Figure Toolbar'),...
	    'Callback','dfield9(''showbar'')');
	dud.menu = menu;
	
	set(dfdisp,'ToolBar','none');
	set(0,'showhiddenhandles',hhsetup);
	set(gcf,'WindowButtonDownFcn','dfield9(''down'')');
	set(dfdisp,'WindowButtonMotionFcn','dfield9(''cdisp'')');
	...
	ppkbd = findobj('name','dfield9 Keyboard input','vis','on');
	if ~isempty(ppkbd),dfield9('kbd'),end
	dfield9('dirfield',dfdisp);	
    end
  ...
  elseif strcmp(action,'dirfield')
    ...
    % Find dfield9 Display and get the user data.
  ...
  elseif strcmp(action,'hotcold')
    ...
    if (findstr(get(mehc,'label'),'inactive'))
      ...
      set(dfdisp,'WindowButtonDownFcn','dfield9(''down'')');
  ...
  elseif strcmp(action,'several')
    ...
    %  set([ch;mh],'enable','on');
    set(ch,'enable','on');
    set(disph,'WindowbuttonDownFcn','dfield9(''down'')');
  ...
  elseif strcmp(action,'kcompute')
    ...
    % button on the dfield9 Keyboard figure.
	...
	if isempty(pv) 
	  ...
	  xstr = dfield9('paraeval',pn,pv,xstr);
	  tstr = dfield9('paraeval',pn,pv,tstr);
	end
      end
    end
    xvalue = str2num(xstr);
    ...
    if compute
      dfield9('solution',initpt,dfdisp);
    end
    nstr = get(dud.notice,'string');
    nstr(1:4) = nstr(2:5);
    nstr{5} = 'Ready.';
  ...
  elseif strcmp(action,'kbd')
    ...
    % sets up the dfield9 Keyboard figure which allows accurate input of 
    % initial values using the keyboard.
    
    ppkbd = findobj('name','dfield9 Keyboard input');
    if ~isempty(ppkbd)
      delete(ppkbd);
    end    
    dfdisp = gcf;
    ...
    ppkbd = figure('name','dfield9 Keyboard input',...
		   'vis','off',...
		   'NumberTitle','off','tag','dfield9');
    
    dfield9('figdefault',ppkbd);
    ...
    kbd.comp = uicontrol('style','push',...		
			 'string','Compute','Callback','dfield9(''kcompute'')');
  ...
  elseif strcmp(action,'zoomin')
    ...
    set(gcf,'WindowButtonDownFcn','dfield9(''zoom'')',...
  ...
  elseif strcmp(action,'solvset')
    ...
    setfig = findobj('name','dfield9 Solver settings');
    if ~isempty(setfig)
    ...
    setfig = figure('name','dfield9 Solver settings','NumberTitle','off',...
		    'tag','dfield9','vis','off');
    
    dfield9('figdefault',setfig);
  ...
  elseif strcmp(action,'settings')
    ...
    % up the dfield9 Settings window, which allows the user to interactively
    ...
    setfig = findobj('name','dfield9 Window settings');
    if ~isempty(setfig)
      close(setfig);
    end
    setfig = figure('name','dfield9 Window settings',...
		    'NumberTitle','off',...
		    'tag','dfield9','vis','off');
  ...
  elseif strcmp(action,'setchange')
    ...
    % on the dfield9 Settings window.
  ...
  elseif strcmp(action,'print')
    ...
    nstr = get(dud.notice,'string');
    nstr(1:4) = nstr(2:5);
    nstr{5} = 'Preparing to print the dfield9 Display Window.  Please be patient.';
    set(dud.notice,'string',nstr);
    
    nstr(1:4) = nstr(2:5);
    nstr{5} = 'Printing the dfield9 Display Window.';
    set(dud.notice,'string',nstr);
  ...
  elseif strcmp(action,'level')
    ...
    xname(find(abs(xname)==92))=[];
    dflevel = findobj('name','dfield9 Level sets');
    if ~isempty(dflevel)
      delete(dflevel)
    end
    dflevel = figure('name','dfield9 Level sets',...
		     'vis','off',...
		     'NumberTitle','off','tag','dfield9');
    
    dfield9('figdefault',dflevel);
    hhsetup = get(0,'showhiddenhandles');
  ...
  elseif strcmp(action,'levcomp')
	...
	if isempty(pv) 
	  ...
	  pv(find(abs(pv)==32))=[];
	  lfcn = dfield9('paraeval',pn,pv,lfcn);
	end
      end
    end
    l = length(lfcn);
  ...
  elseif strcmp(action,'showbar')
    ...
    if strcmp(state,'figure')
	  ...
	  'clickedcallback',['dfield9(''zoomback'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Zoom In'),...
	  'clickedcallback',['dfield9(''zoomin'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Print'),...
	  'clickedcallback','dfield9(''print'');');
  ...
  elseif strcmp(action,'figdefault')
    ...
    fig = input1;
    set(fig,'CloseRequestFcn','dfield9(''closefcn'')');
    dfset = findobj('name','dfield9 Setup');
    sud = get(dfset,'user');
    ud = get(fig,'user');
    ud.ssize = sud.ssize;
  ...
  elseif strcmp(action,'closefcn')
    ...
    fig = gcf;  
    name = get(fig,'name');
    if strcmp(name,'dfield9 Setup') | strcmp(name,'dfield9 Display')
      quest = ['Closing this window will cause all dfield9 ',...
	       'windows to close, and dfield9 will stop.  ',...
	       'Do you want to quit dfield9?'];
      butt = questdlg(quest,'Quit dfield9?','Quit','Cancel','Quit');
      if strcmp(butt,'Quit')
	dfield9('quit');
      end
    elseif strcmp(name,'dfield9 Linearization')
      dud = get(fig,'user');
      fcn = dud.function;
	if (exist(fcn)==2) delete([fcn,'.m']);end    
  ...
  elseif strcmp(action,'confused')
    ...
    tstring = 'dfield9 is totally confused';
    qstring = {['You will have to restart dfield9 from '...
		...
		'it would be best to do nothing now.'];
	       'What do you want to do?'};
    bstr1 = 'Quit and restart dfield9.';
    bstr2 = 'Just quit dfield9.';
    bstr3 = 'Do nothing.';
    answer = questdlg(qstring,tstring,bstr1,bstr2,bstr3,bstr1);
    if strcmp(answer,bstr1)
      delete(findobj('tag','dfield9'));
      dfield9;return
  ...
  elseif strcmp(action,'loadsyst')  % This loads either a system or a gallery.
	...
	errmsg = 'This is not a dfield9 file.';
	fprintf('\a')
	errordlg(errmsg,'dfield9 error','on');
	return
      end
    ...
    for j = 1:nnn
      ...
      newsyst.name = sname;
      ignore = dfield9('addgall',newsyst);
      if ignore == -1;
...
aa = v(1)+0.01*(v(2)-v(1));
plh = plot(aa,y0,'.','markersize',20,'color',col,...
    'parent',dispha,'visible',vi);    %'erase','xor',
ud.pline = plh;
set(dispha,'UserData',ud);
