function output = pplane10(action,input1,input2,input3)

% pplane10  is an interactive tool for studying planar autonomous systems of
%	differential equations.  When pplane10 is executed, a pplane10 Setup
...
%
%	When the Proceed button is pressed on the Setup window, the pplane10
...
%	the pplane10 Display window, the solution to the system with that
...
if strcmp(action,'initialize')
   ...
   % First we make sure that there is no other copy of pplane10
   % running, since this causes problems.	
   pph = findobj('tag','pplane10');
   if ~isempty(pph);
      qstring = {'There are some pplane10 figures open although they may be invisible.  ';...
            'What do you want to do?'};
      tstring = 'Only one copy of pplane10 can be open at one time.';
      b1str = 'Restart pplane10.';
   ...
   % Next we look for old files created by pplane10.
   ...
   for k = 1:length(oldfiles)
      ...
      ll = fgetl(fid);
      fclose(fid);
      if strcmp(ll,'%% Created by pplane10')
         delete(fn)
      else
	l= length(ofiles);
	ofiles{l+1} = fn;
  ...
  ud.color = color;
  ud.level = ' ';
  ppset = figure('name','pplane10 Setup',...
		 'tag','pplane10','visible','off',...
		 'user',ud);
  
  pplane10('figdefault',ppset);
  ...
  butt(2) = uicontrol('style','push',...
		      'pos',rwind,...
		      'string','Revert',...
		      'call','pplane10(''revert'')',...
		      'visible','off');
  
  butt(3) = uicontrol('style','push',...
		      'pos',pwind,...
		      'string','Proceed',...
		      'call','pplane10(''proceed'')',...
		      'visible','off');
  ...
  callnfpts = [
      'ppset = findobj(''name'',''pplane10 Setup'');',...
      'ud = get(ppset,''user'');'...
      'me = ud.h.npts;',...
  ...
  meload = uimenu(mefile,'label','Load a system ...',...
		  'MenuSelectedFcn','pplane10(''loadsyst'',''system'');',...
		  'pos',1);
  mesave = uimenu(mefile,'label','Save the current system ...',...
		  'MenuSelectedFcn','pplane10(''savesyst'',''system'');',...
		  'pos',2);
  meloadg = uimenu(mefile,'label','Load a gallery ...',...
		   'MenuSelectedFcn','pplane10(''loadsyst'',''gallery'');',...
		   'separator','on','pos',3);
  mesaveg = uimenu(mefile,'label','Save a gallery ...',...
		   'MenuSelectedFcn','pplane10(''savesyst'',''gallery'');',...
		   'tag','savegal','pos',4);
  medelg = uimenu(mefile,'label','Delete the current gallery',...
		  'MenuSelectedFcn',delgall,'pos',5);
  melddg = uimenu(mefile,'label','Load the default gallery',...
		  'MenuSelectedFcn','pplane10(''loadsyst'',''default'');',...
		  'enable','on',...
		  'tag','load default','pos',6);
  merevert = uimenu(mefile,'label','Revert','MenuSelectedFcn',...
		    'pplane10(''revert'')',...
		    'separator','on','pos',7);
  meproceed = uimenu(mefile,...
		     'label','Proceed',...
		     'MenuSelectedFcn','pplane10(''proceed'')',...
		     'separator','off',...
		     'accelerator','G','pos',8);
  ...
  merestart = uimenu(mefile,'label',...
		     'Restart pplane10',...
		     'MenuSelectedFcn','pplane10(''restart'')',...
		     'separator','on','pos',12);
  
  mequit = uimenu(mefile,...
		  'label','Quit pplane10',...
		  'MenuSelectedFcn','pplane10(''quit'')',...
  ...
  meclrf = uimenu(meedit,'label','Clear equations',...
		  'MenuSelectedFcn',['ud = get(gcf,''user'');h = ud.h;',...
  ...
  meclrp = uimenu(meedit,'label','Clear parameters',...
      'MenuSelectedFcn',pclear,...
      'accelerator','N');
  
  meclrwind = uimenu(meedit,'label','Clear display window',...
      'MenuSelectedFcn',['ud = get(gcf,''user'');',...
  ...
  % Gallery menu.
   ...
   meadd = uimenu(sysmenu,'label','Add current system to the gallery',...
      'MenuSelectedFcn','pplane10(''addgall'');','tag','add system');
   sep = 'on';
   for kk = 1:length(system)
      kkk = num2str(kk);
      if kk == 2, sep = 'off';end
      sysmen(kk) = uimenu(sysmenu,'label',system(kk).name,...
         'MenuSelectedFcn',['pplane10(''system'',',kkk,')'],...
...
elseif strcmp(action,'loadsyst')  % This loads either a system or a gallery.
   ...
   for j = 1:nnn
      ...
      newsyst.name = sname;
      ignore = pplane10('addgall',newsyst);
      if ignore == -1;
...
elseif strcmp(action,'proceed')
   ...
   sud.o = sud.c;
   set(ppset,'user',sud);
   he = findobj('name','pplane10 Equilibrium point data');
   hl = findobj('name','pplane10 Linearization');
   close([he;hl]);
   % set([he;hl],'vis','off');
   ...
   if (sud.flag == 1)
     ...
     NumbFPts = sud.c.npts;
     ppdisp = findobj('name','pplane10 Display');
     dud = get(ppdisp,'user');
...
%       set([arr.hx;arr.hy;arr.barrows],'vis','on');
	 ...
	 if isempty(pv) 
	   ...
	   if isempty(str2num(pv)) % This is an expression.
	     ...
	     if (err | isempty(pval))
	       ...
	       fprintf('\a')
	       errordlg(errmsg,'pplane10 error','on');
	       warning(SS)
	       return
	     end
	   end
	   xxderivstr = pplane10('paraeval',pn,pv,xderivstr);
	   yyderivstr = pplane10('paraeval',pn,pv,yderivstr);
     ...
     % Find pplane10 Display if it exists.	
     % If pplane10 Display exists, update it.  If it does not build it.
	 ...
	 if ~isempty(menull)
       ...
       ppdisp = figure('name','pplane10 Display',...
		       ...
		       'interrupt','on',...
		       'visible','off',...
		       'tag','pplane10');
       pplane10('figdefault',ppdisp);
       dud = get(ppdisp,'user');
       dud.syst = sud.c;
	...
	otherwise
	 ...
	 set(gcf,'defaultuicontrolfontsize',fs*0.9);
       ...
       merestart = uimenu(fmenu,'label',...
			  ...
			  'MenuSelectedFcn','pplane10(''restart'')',...
			  'separator','on');
       mequit = uimenu(fmenu,'label','Quit pplane10',...
		       'MenuSelectedFcn','pplane10(''quit'')','separator','off');
	 ...
	 metest = uimenu(solmenu,'label','Test case',...
       ...
       meset   = uimenu(menu(1),'label','Settings.',...
			'MenuSelectedFcn','pplane10(''settings'')');
	...
	ode113ch = 'off';
	 ...
	 ode23tbch = 'off';
       ...
       dud.solver(1) = uimenu(mesolve,'label','Dormand Prince',...
			      'checked',dpch,...
			      'MenuSelectedFcn',solverstr,'user',dpset);
       
       dud.solver(2) = uimenu(mesolve,'label','Runge-Kutta 4',...
			      'checked',rk4ch,...
			      'MenuSelectedFcn',solverstr,'user',rk4set);
       
       dud.solver(3) = uimenu(mesolve,'label','ode45',...
			      'checked',ode45ch,...
			      'separator','on',...
			      'MenuSelectedFcn',solverstr,'user',ode45set);
       
       dud.solver(4) = uimenu(mesolve,'label','ode23',...
			      'checked',ode23ch,...
			      'MenuSelectedFcn',solverstr,'user',ode23set);
       
       dud.solver(5) = uimenu(mesolve,'label','ode113',...
			      'checked',ode113ch,...
			      'MenuSelectedFcn',solverstr,'user',ode113set);
       
       dud.solver(6) = uimenu(mesolve,'label','ode15s',...
			      'checked',ode15sch,...
			      'MenuSelectedFcn',solverstr,'user',ode15sset);
       
       dud.solver(7) = uimenu(mesolve,'label','ode23s',...
			      'checked',ode23sch,...
			      'MenuSelectedFcn',solverstr,'user',ode23sset);
       
       dud.solver(8) = uimenu(mesolve,'label','ode23t',...
			      'checked',ode23tch,...
			      'MenuSelectedFcn',solverstr,'user',ode23tset);
       
       dud.solver(9) = uimenu(mesolve,'label','ode23tb',...
			      'checked',ode23tbch,...
			      'MenuSelectedFcn',solverstr,'user',ode23tbset);
	   ...
	   'MenuSelectedFcn','pplane10(''showbar'')');
       
       set(0,'showhiddenhandles',hhsetup);
       
       set(gcf,'WindowButtonDownFcn','pplane10(''down'')');
       set(ppdisp,'WindowButtonMotionFcn','pplane10(''cdisp'')');
	...
	case 6
     ...
     ppkbd = findobj('name','pplane10 Keyboard input','vis','on');
     if ~isempty(ppkbd),pplane10('kbd'),end
     pplevel = findobj('name','pplane10 Level sets');
...
elseif strcmp(action,'linear')
   ...
   % Get the information from pplane10 Equilibrium ...
   ...
   % Find pplane10 Linearization if it exists.	
   % If pplane10 Linearization exists, update it.  If it does not build it.
   
   pplin= findobj('name','pplane10 Linearization');
   if (~isempty(pplin))
      figure(pplin);
      dud = get(pplin,'user');
      dud.syst = system;
      ...
      pplin = figure('name','pplane10 Linearization',...
         ...
         'interrupt','on',...
         'visible','off',...
         'tag','pplane10');
      pplane10('figdefault',pplin);
      dud = get(pplin,'user');
      dud.dir = 0;
      dud.syst = system;
      dud.settings = settings;
      ...
      mequit = uimenu(fmenu,'label','Quit pplane10',...
	  'MenuSelectedFcn','pplane10(''quit'')','separator','on','pos',9);
      ...
      dud.menu = uimenu(solmenu,'MenuSelectedFcn',fundcall);
	...
	chkd = 'on';
      ...
      set(pplin.Number,'WindowButtonDownFcn',['pplane10(''down'',',num2str(pplin.Number),')']);
...
elseif strcmp(action,'dirfield')
   ...
   % Find pplane10 Display and get the user data.
   ...
   notice = dud.notice;
   if ~isempty(notice) && notice~=0 && notice~=0
   ...
   N = dud.syst.npts;
   if strcmp(get(disph,'name'),'pplane10 Linearization') 
      N = floor(3*N/4);
   end    
...
elseif strcmp(action,'hotcold')
   ...
   if (findstr(get(mehc,'label'),'inactive'))
      ...
      set(ppdisp,'WindowButtonDownFcn','pplane10(''down'')');
...
elseif strcmp(action,'down')
  ...
  % 'down' is the Window Button Down MenuSelectedFcn.  It starts the computation of
  ...
  pplane10('solution',initpt,disph);
...
elseif strcmp(action,'test case')
   ...
   for k = 1:10
      initpt = k*[-.2 .2];
      pplane10('solution',initpt,ppdisp);
      initpt = k*[-.2 -.2];
      pplane10('solution',initpt,ppdisp);
   end  
   pplane10('solution',[1,-1],ppdisp);
   dud = get(gcf,'user');
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5,1} = 'Ready.';
...
elseif strcmp(action,'kcompute')
   ...
   % 'kcompute' is the MenuSelectedFcn back for the Compute 
   % button on the pplane10 Keyboard figure.
   ...
   for kk = 1:6;
	 ...
	 xstr = pplane10('paraeval',pn,pv,xstr);
	 ystr = pplane10('paraeval',pn,pv,ystr);
       end
     end
   end
   xvalue = str2num(xstr);
...
elseif strcmp(action,'kbd')
   ...
   % sets up the pplane10 Keyboard figure which allows accurate input of 
   ...
   ppkbd = findobj('name','pplane10 Keyboard input');
   if  ~isempty(ppkbd)
     delete(ppkbd);
   end
         
   ppkbd = figure('name','pplane10 Keyboard input',...
		  'vis','off',...
		  'tag','pplane10');
   
   pplane10('figdefault',ppkbd);
   ...
   kbd.comp = uicontrol('style','push',...		
			'string','Compute','call','pplane10(''kcompute'')');
...
%  z0 = ppginput(1);
  ...
  ppeqpt = findobj('name','pplane10 Equilibrium point data');
  
   if (isempty(ppeqpt))
     ppeqpt = figure('vis','off','resize','on',...
 		    'name','pplane10 Equilibrium point data',...
 		    'tag','pplane10');
     pplane10('figdefault',ppeqpt);
...
elseif strcmp(action,'zoomin')
   ...
   set(gcf,'WindowButtonDownFcn','pplane10(''zoom'')',...
...
elseif strcmp(action,'zoom')
   ...
   set(axh,'user',aud);	
   set(disph,'user',dud);
   set(disph,'WindowButtonDownFcn','pplane10(''down'')',...
      'WindowButtonUpFcn','');
   pplane10('dirfield',disph);
   ppset = findobj('name','pplane10 Setup');
   if isempty(ppset)
      pplane10('confused');
   else
...
elseif strcmp(action,'showbar')
  ...
  if strcmp(state,'figure')
	...
	'clickedcallback','pplane10(''print'');');
    switch name
     case 'pplane10 Display'
      set(findobj(sbfig,'tooltipstr','Zoom Out'),...
	  'clickedcallback',['pplane10(''zoomback'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Zoom In'),...
	  'clickedcallback',['pplane10(''zoomin'');' fixtb]);
      delete(findobj(sbfig,'tooltipstr','Rotate 3D'));
      
     case 'pplane10 Linearization'
...
elseif strcmp(action,'settings')
   ...
   % up the pplane10 Settings window, which allows the user to interactively
   ...
   setfig = findobj('name','pplane10 Settings');
   if ~isempty(setfig);
     delete(setfig);
   end
   
   setfig = figure('name','pplane10 Settings',...
		   ...
		   'tag','pplane10','vis','off');
   
   pplane10('figdefault',setfig);
...
elseif strcmp(action,'setchange')
   ...
   % on the pplane10 Settings window.
...
elseif strcmp(action,'delete')
   ...
   trjh = dud.solhand;
   notice = dud.notice;
   if ~isempty(notice) && notice~=0 && notice~=0   % Display window
      nstr = get(notice,'string');
      nstr(1:4) = nstr(2:5);
...
elseif strcmp(action,'plotxy')  % Start a graph 
  ...
  x=get(objh,'xdata');
  y=get(objh,'ydata');
  hh = pplane10('plotxyfig',type,gcf);
  set(ppdisp,'WindowButtonDownFcn',oldcall);
  aud = get(hh(2),'user');
  ud = get(hh(1),'user');
  ud.data = [t;x;y];
  set(hh(1),'user',ud);
  pplane10('plxy',aud.rad,hh(1));
...
elseif strcmp(action,'plotxyfig')  % Build the graph figure.
  ...
  ppxy = figure('tag','pplane10','visible','off');
  name = ['pplane10 t-plot # ',int2str(ppxy.Number)];
  ud.color = dud.color;
  ud.type = 0;
  set(ppxy,'name',name,'user',ud);
  pplane10('figdefault',ppxy);
  ...
  set(findobj(fmenu,'label','Print Pre&view...'),'pos',6);
  set(findobj(fmenu,'label','&Print...'),'pos',7);
  qmenu = uimenu(fmenu,'label','Quit pplane10',...
		 'MenuSelectedFcn','pplane10(''quit'')',...
		 'separator','on');
...
elseif strcmp(action,'plxy')
   ...
   switch type
    case 1 
      ...
      ylabel(xstring)
      set(r3dh,'vis','off')
      set(fig,'WindowButtonDownFcn','pplane10(''plxybdf'')');
    case 2
...
elseif strcmp(action,'crop')
   ...
   hh = pplane10('plotxyfig',type,ppxy);
   ud = get(hh(1),'user');
   ud.data = [t;x;y];
   set(hh(1),'user',ud);
   aud = get(hh(2),'user');
...
elseif strcmp(action,'plxybdf')
   ...
   dispa = gca;
   dispf = gcf;
   set(dispf,'windowbuttonmotionfcn','pplane10(''plxybmf'')',...
      'windowbuttonupfcn','pplane10(''plxybuf'')',...
      'inter','on');
   aud = get(dispa,'user');
   delete(aud.h);
   aud.h = [];
...
elseif strcmp(action,'print')
   ...
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Preparing to print the pplane10 Display Window.  Please be patient.';
   set(dud.notice,'string',nstr);
   
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Printing the pplane10 Display Window.';
   set(dud.notice,'string',nstr);
...
elseif strcmp(action,'figdefault')
  ...
  fig = input1;
  set(fig,'CloseRequestFcn','pplane10(''closefcn'')');
  ppset = findobj('name','pplane10 Setup');
  sud = get(ppset,'user');
  ud = get(fig,'user');
  ud.ssize = sud.ssize;
...
elseif strcmp(action,'zoominsq')
   ...
   set(gcf,'WindowButtonDownFcn','pplane10(''zoomsqd'')',...
      'WindowButtonUpFcn','1;','inter','on');
   set(gca,'inter','on');
   dud = get(gcf,'user');
   nstr = get(dud.notice,'string');
...
elseif strcmp(action,'zoomsqd')
   ...
   set(ppdispa,'user',aud);
   set(gcf,'windowbuttonmotionfcn','pplane10(''zoomsqm'')',...
      'windowbuttonupfcn','pplane10(''zoomsqu'')');
...
elseif strcmp(action,'zoomsqu')
   ...
   set(disph,'user',dud);
   set(disph,'WindowButtonDownFcn','pplane10(''down'')',...
      'WindowButtonMotionFcn','pplane10(''cdisp'')',...
      'WindowButtonUpFcn','');
   pplane10('dirfield',disph);
   ppset = findobj('name','pplane10 Setup');
   if isempty(ppset)
...
elseif strcmp(action,'level')
  ...
  pplevel = figure('name','pplane10 Level sets',...
		 'vis','off',...
		 'tag','pplane10');
  
  pplane10('figdefault',pplevel);
  ...
  set(lev.ch,'MenuSelectedFcn',callrad);
...
elseif strcmp(action,'levcomp')
  ...
  for kk = 1:4;
	...
	pv(find(abs(pv)==32))=[];
	lfcn = pplane10('paraeval',pn,pv,lfcn);
      end
    end
  end
  l = length(lfcn);
...
elseif strcmp(action,'closefcn')
   ...
   fig = gcf;  
   name = get(fig,'name');
   if strcmp(name,'pplane10 Setup') | strcmp(name,'pplane10 Display')
      quest = ['Closing this window will cause all pplane10 ',...
            'windows to close, and pplane10 will stop.  ',...
            'Do you want to quit pplane10?'];
      butt = questdlg(quest,'Quit pplane10?','Quit','Cancel','Quit');
      if strcmp(butt,'Quit')
         pplane10('quit');
      end
   elseif strcmp(name,'pplane10 Linearization')
      dud = get(fig,'user');
      fcn = dud.function;
...
elseif strcmp(action,'confused')
   ...
   tstring = 'pplane10 is totally confused';
   qstring = {['You will have to restart pplane10 from '...
      ...
      'What do you want to do?'};
   bstr1 = 'Quit and restart pplane10.';
   bstr2 = 'Just quit pplane10.';
   bstr3 = 'Do nothing.';
   answer = questdlg(qstring,tstring,bstr1,bstr2,bstr3,bstr1);
   if strcmp(answer,bstr1)
      delete(findobj('tag','pplane10'));
      pplane10;return
...
% Input the user data. 
  ...
  yout = yout(1:N,:);
try
  if ~isempty(dud.notice) & dud.noticeflag
    nstr = get(dud.notice,'string');
...
% PPOUT  is the output function for pplane10.

%  Copywright (c)  John C. Polking, Rice University
%  Last Modified: January 21, 2001

output = 0;
ppdisp = findobj(get(0,'child'),'flat','name','pplane10 Display'); %gca;   % 
dud = get(ppdisp,'user');
ppdispa = dud.axes;
dfcn = dud.function;
ud = get(ppdispa,'user');
