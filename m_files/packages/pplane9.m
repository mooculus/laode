function output = pplane9(action,input1,input2,input3)

% pplane9  is an interactive tool for studying planar autonomous systems of
%	differential equations.  When pplane9 is executed, a pplane9 Setup
...
%
%	When the Proceed button is pressed on the Setup window, the pplane9
...
%	the pplane9 Display window, the solution to the system with that
...
%
%  This is a modified version to work with new Matlab versions
...
if strcmp(action,'initialize')
   ...
   % First we make sure that there is no other copy of pplane9
   % running, since this causes problems.	
   pph = findobj('tag','pplane9');
   if ~isempty(pph);
      qstring = {'There are some pplane9 figures open although they may be invisible.  ';...
            'What do you want to do?'};
      tstring = 'Only one copy of pplane9 can be open at one time.';
      b1str = 'Restart pplane9.';
   ...
   % Next we look for old files created by pplane9.
   ...
   for k = 1:length(oldfiles)
      ...
      ll = fgetl(fid);
      fclose(fid);
      if strcmp(ll,'%% Created by pplane9')
         delete(fn)
      else
	l= length(ofiles);
	ofiles{l+1} = fn;
  ...
  ud.color = color;
  ud.level = ' ';
  ppset = figure('name','pplane9 Setup','NumberTitle','off',...
		 'tag','pplane9','visible','off',...
		 'UserData',ud);
  
  pplane9('figdefault',ppset);
  ...
  butt(2) = uicontrol('style','push',...
		      'pos',rwind,...
		      'string','Revert',...
		      'Callback','pplane9(''revert'')',...
		      'visible','off');
  
  butt(3) = uicontrol('style','push',...
		      'pos',pwind,...
		      'string','Proceed',...
		      'Callback','pplane9(''proceed'')',...
		      'visible','off');
  ...
  callnfpts = [
      'ppset = findobj(''name'',''pplane9 Setup'');',...
  ...
  melddg = uimenu(mefile,'label','Load the default gallery',...
		  'Callback','pplane9(''loadsyst'',''default'');',...
		  'enable','on',...
  ...
  merestart = uimenu(mefile,'label',...
		     'Restart pplane9',...
		     'Callback','pplane9(''restart'')',...
		     'separator','on','pos',12);
  
  mequit = uimenu(mefile,...
		  'label','Quit pplane9',...
		  'Callback','pplane9(''quit'')',...
...
elseif strcmp(action,'loadsyst')  % This loads either a system or a gallery.
   ...
   for j = 1:nnn
      ...
      newsyst.name = sname;
      ignore = pplane9('addgall',newsyst);
      if ignore == -1;
...
elseif strcmp(action,'proceed')
   ...
   sud.o = sud.c;
   set(ppset,'UserData',sud);
   he = findobj('name','pplane9 Equilibrium point data');
   hl = findobj('name','pplane9 Linearization');
   close([he;hl]);
   % set([he;hl],'vis','off');
   ...
   if (sud.flag == 1)
     ...
     NumbFPts = sud.c.npts;
     ppdisp = findobj('name','pplane9 Display');
...
%       set([arr.hx;arr.hy;arr.barrows],'vis','on');
	 ...
	 if isempty(pv) 
	   ...
	   if isempty(str2num(pv)) % This is an expression.
	     ...
	     if (err | isempty(pval))
	       ...
	       fprintf('\a')
	       errordlg(errmsg,'pplane9 error','on');
	       warning(SS)
	       return
	     end
	   end
	   xxderivstr = pplane9('paraeval',pn,pv,xderivstr);
	   yyderivstr = pplane9('paraeval',pn,pv,yderivstr);
     ...
     % Find pplane9 Display if it exists.	
     % If pplane9 Display exists, update it.  If it does not build it.
	 ...
	 if ~isempty(menull)
       ...
       ppdisp = figure('name','pplane9 Display',...
		       'NumberTitle','off',...
		       'interrupt','on',...
		       'visible','off',...
		       'tag','pplane9');
       pplane9('figdefault',ppdisp);
       dud = get(ppdisp,'UserData');
       dud.syst = sud.c;
	...
	otherwise
	 ...
	 set(gcf,'defaultuicontrolfontsize',fs*0.9);
       ...
       merestart = uimenu(fmenu,'label',...
			  ...
			  'Callback','pplane9(''restart'')',...
			  'separator','on');
       mequit = uimenu(fmenu,'label','Quit pplane9',...
		       'Callback','pplane9(''quit'')','separator','off');
       ...
       menu(2) = uimenu(emenu,'label','Zoom in.',...
			'Callback','pplane9(''zoomin'')',...
			'pos',1);
       
       zsqmenu = uimenu(emenu,'label','Zoom in square.',...
			'Callback','pplane9(''zoominsq'')',...
			'pos',2);
	...
	ode113ch = 'off';
	 ...
	 ode23tbch = 'off';
       ...
       menu(6) = uimenu(menu(1),'label','Make the Display Window inactive.',...
			'Callback','pplane9(''hotcold'')','separator','on');
	   ...
	   'Callback','pplane9(''showbar'')');
       
       set(0,'showhiddenhandles',hhsetup);
       
       set(gcf,'WindowButtonDownFcn','pplane9(''down'')');
       set(ppdisp,'WindowButtonMotionFcn','pplane9(''cdisp'')');
	...
	case 6
     ...
     ppkbd = findobj('name','pplane9 Keyboard input','vis','on');
     if ~isempty(ppkbd),pplane9('kbd'),end
     pplevel = findobj('name','pplane9 Level sets');
...
elseif strcmp(action,'linear')
   ...
   % Get the information from pplane9 Equilibrium ...
   ...
   % Find pplane9 Linearization if it exists.	
   % If pplane9 Linearization exists, update it.  If it does not build it.
   
   pplin= findobj('name','pplane9 Linearization');
   if (~isempty(pplin))
      figure(pplin);
      ...
      pplin = figure('name','pplane9 Linearization',...
         'NumberTitle','off',...
         'interrupt','on',...
         'visible','off',...
         'tag','pplane9');
      pplane9('figdefault',pplin);
      dud = get(pplin,'UserData');
      dud.dir = 0;
      dud.syst = system;
      dud.settings = settings;
      ...
      dud.butt  = uicontrol('style','push',...
         'pos',stoppos,...
         'string','Stop','Callback',stopstr,'vis','off','tag','stop');
      ...
      mequit = uimenu(fmenu,'label','Quit pplane9',...
	  'Callback','pplane9(''quit'')','separator','on','pos',9);
	...
	chkd = 'on';
      ...
      set(0,'showhiddenhandles',hhsetup);
      %pplin %JH display for debugging
      set(pplin,'WindowButtonDownFcn',['pplane9(''down'',',num2str(pplin.Number),')']);
...
elseif strcmp(action,'dirfield')
   ...
   % Find pplane9 Display and get the user data.
   ...
   N = dud.syst.npts;
   if strcmp(get(disph,'name'),'pplane9 Linearization') 
      N = floor(3*N/4);
   end    
...
elseif strcmp(action,'hotcold')
   ...
   if (findstr(get(mehc,'label'),'inactive'))
      ...
      set(ppdisp,'WindowButtonDownFcn','pplane9(''down'')');
...
elseif strcmp(action,'test case')
   ...
   for k = 1:10
      initpt = k*[-.2 .2];
      pplane9('solution',initpt,ppdisp);
      initpt = k*[-.2 -.2];
      pplane9('solution',initpt,ppdisp);
   end  
   pplane9('solution',[1,-1],ppdisp);
   dud = get(gcf,'UserData');
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5,1} = 'Ready.';
...
elseif strcmp(action,'kcompute')
   ...
   % button on the pplane9 Keyboard figure.
   ...
   for kk = 1:6;
	 ...
	 xstr = pplane9('paraeval',pn,pv,xstr);
	 ystr = pplane9('paraeval',pn,pv,ystr);
       end
     end
   end
   xvalue = str2num(xstr);
...
elseif strcmp(action,'kbd')
   ...
   % sets up the pplane9 Keyboard figure which allows accurate input of 
   ...
   ppkbd = findobj('name','pplane9 Keyboard input');
   if  ~isempty(ppkbd)
     delete(ppkbd);
   end
         
   ppkbd = figure('name','pplane9 Keyboard input',...
		  'vis','off',...
		  'NumberTitle','off','tag','pplane9');
   
   pplane9('figdefault',ppkbd);
   ...
   kbd.comp = uicontrol('style','push',...		
			'string','Compute','Callback','pplane9(''kcompute'')');
...
%  z0 = ppginput(1);
  ...
  ppeqpt = findobj('name','pplane9 Equilibrium point data');
  
   if (isempty(ppeqpt))
     ppeqpt = figure('vis','off','resize','on',...
 		    'name','pplane9 Equilibrium point data',...
 		    'NumberTitle','off','tag','pplane9');
     pplane9('figdefault',ppeqpt);
...
elseif strcmp(action,'zoomin')
   ...
   set(gcf,'WindowButtonDownFcn','pplane9(''zoom'')',...
...
elseif strcmp(action,'zoom')
   ...
   set(disph,'UserData',dud);
   set(disph,'WindowButtonDownFcn','pplane9(''down'')',...
      'WindowButtonUpFcn','');
   pplane9('dirfield',disph);
   ppset = findobj('name','pplane9 Setup');
   if isempty(ppset)
      pplane9('confused');
   else
...
elseif strcmp(action,'showbar')
  ...
  if strcmp(state,'figure')
	...
	'clickedcallback','pplane9(''print'');');
    switch name
     case 'pplane9 Display'
      set(findobj(sbfig,'tooltipstr','Zoom Out'),...
	  'clickedcallback',['pplane9(''zoomback'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Zoom In'),...
	  'clickedcallback',['pplane9(''zoomin'');' fixtb]);
      delete(findobj(sbfig,'tooltipstr','Rotate 3D'));
      
     case 'pplane9 Linearization'
...
elseif strcmp(action,'settings')
   ...
   % up the pplane9 Settings window, which allows the user to interactively
   ...
   setfig = findobj('name','pplane9 Settings');
   if ~isempty(setfig);
     delete(setfig);
   end
   
   setfig = figure('name','pplane9 Settings',...
		   'NumberTitle','off',...
		   'tag','pplane9','vis','off');
   
   pplane9('figdefault',setfig);
   ...
   data.nstep = uicontrol('style','edit','Callback',nstepcall,...
...
elseif strcmp(action,'setchange')
   ...
   % on the pplane9 Settings window.
...
elseif strcmp(action,'plotxyfig')  % Build the graph figure.
  ...
  ppxy = figure('number','off',...
		'tag','pplane9','visible','off');
  name = ['pplane9 t-plot # ',int2str(ppxy)];
  ud.color = dud.color;
  ud.type = 0;
  set(ppxy,'name',name,'UserData',ud);
  pplane9('figdefault',ppxy);
  ...
  set(findobj(fmenu,'label','Print Pre&view...'),'pos',6);
  set(findobj(fmenu,'label','&Print...'),'pos',7);
  qmenu = uimenu(fmenu,'label','Quit pplane9',...
		 'Callback','pplane9(''quit'')',...
		 'separator','on');
...
elseif strcmp(action,'plxy')
   ...
   switch type
    case 1 
      ...
      ylabel(xstring)
      set(r3dh,'vis','off')
      set(fig,'WindowButtonDownFcn','pplane9(''plxybdf'')');
    case 2
...
elseif strcmp(action,'crop')
   ...
   hh = pplane9('plotxyfig',type,ppxy);
   ud = get(hh(1),'UserData');
   ud.data = [t;x;y];
   set(hh(1),'UserData',ud);
   aud = get(hh(2),'UserData');
...
elseif strcmp(action,'plxybdf')
   ...
   dispa = gca;
   dispf = gcf;
   set(dispf,'windowbuttonmotionfcn','pplane9(''plxybmf'')',...
      'windowbuttonupfcn','pplane9(''plxybuf'')',...
      'inter','on');
...
elseif strcmp(action,'print')
   ...
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Preparing to print the pplane9 Display Window.  Please be patient.';
   set(dud.notice,'string',nstr);
   
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Printing the pplane9 Display Window.';
   set(dud.notice,'string',nstr);
...
elseif strcmp(action,'figdefault')
  ...
  fig = input1;
  set(fig,'CloseRequestFcn','pplane9(''closefcn'')');
  ppset = findobj('name','pplane9 Setup');
  sud = get(ppset,'UserData');
  ud = get(fig,'UserData');
  ud.ssize = sud.ssize;
...
elseif strcmp(action,'zoominsq')
   ...
   set(gcf,'WindowButtonDownFcn','pplane9(''zoomsqd'')',...
      'WindowButtonUpFcn','1;','inter','on');
   set(gca,'inter','on');
   dud = get(gcf,'UserData');
   nstr = get(dud.notice,'string');
...
elseif strcmp(action,'zoomsqd')
   ...
   set(ppdispa,'UserData',aud);
   set(gcf,'windowbuttonmotionfcn','pplane9(''zoomsqm'')',...
      'windowbuttonupfcn','pplane9(''zoomsqu'')');
...
elseif strcmp(action,'zoomsqu')
   ...
   set(disph,'UserData',dud);
   set(disph,'WindowButtonDownFcn','pplane9(''down'')',...
      'WindowButtonMotionFcn','pplane9(''cdisp'')',...
      'WindowButtonUpFcn','');
   pplane9('dirfield',disph);
   ppset = findobj('name','pplane9 Setup');
   if isempty(ppset)
...
elseif strcmp(action,'level')
  ...
  pplevel = figure('name','pplane9 Level sets',...
		 'vis','off',...
		 'NumberTitle','off','tag','pplane9');
  
  pplane9('figdefault',pplevel);
...
elseif strcmp(action,'levcomp')
  ...
  for kk = 1:4;
	...
	pv(find(abs(pv)==32))=[];
	lfcn = pplane9('paraeval',pn,pv,lfcn);
      end
    end
  end
  l = length(lfcn);
...
elseif strcmp(action,'closefcn')
   ...
   fig = gcf;  
   name = get(fig,'name');
   if strcmp(name,'pplane9 Setup') | strcmp(name,'pplane9 Display')
      quest = ['Closing this window will cause all pplane9 ',...
            'windows to close, and pplane9 will stop.  ',...
            'Do you want to quit pplane9?'];
      butt = questdlg(quest,'Quit pplane9?','Quit','Cancel','Quit');
      if strcmp(butt,'Quit')
         pplane9('quit');
      end
   elseif strcmp(name,'pplane9 Linearization')
...
elseif strcmp(action,'confused')
   ...
   tstring = 'pplane9 is totally confused';
   qstring = {['You will have to restart pplane9 from '...
      ...
      'What do you want to do?'};
   bstr1 = 'Quit and restart pplane9.';
   bstr2 = 'Just quit pplane9.';
   bstr3 = 'Do nothing.';
   answer = questdlg(qstring,tstring,bstr1,bstr2,bstr3,bstr1);
   if strcmp(answer,bstr1)
      delete(findobj('tag','pplane9'));
      pplane9;return
...
% PPOUT  is the output function for pplane9.

%  Copywright (c)  John C. Polking, Rice University
%  Last Modified: January 21, 2001

output = 0;
ppdisp = findobj(get(0,'child'),'flat','name','pplane9 Display'); %gca;   % 
dud = get(ppdisp,'UserData');
ppdispa = dud.axes;
dfcn = dud.function;
